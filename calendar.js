...
(function (global) {
    ...
    function createMonthTable(data, option) {
        ...
        while (c <= 6) {
            ...
            count = count + 1;
            c = c + 1;
			td.setAttribute("onClick", "tdClick("+(count)+")");
        }
        table.appendChild(tr);
...
function tdClick(val)
{
	var news = val-1;
	var chekcDate = news+"-"+chk+"-"+chk2;

	var cl = document.querySelectorAll('.dycalendar-target-date');
	[].forEach.call(cl,function(el){
		el.classList.remove("dycalendar-target-date");
	})
	
	
	document.getElementById(val).classList.add("dycalendar-target-date");
	 
	 document.getElementsByClassName("js-todo-input2")[0].value = chekcDate;
